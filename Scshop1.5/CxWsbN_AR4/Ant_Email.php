...
<div class="ant"><div class="topdiv"><span class="lft"><i class="fa fa-home" aria-hidden="true"></i> <a href='Ant_mid.php'>首页</a> - 订阅管理</span> </div>
...
<div class="ant">
	<div class="ant_title" style="position: relative;">订阅管理<font class="cl">（带星*的必填）</font>
			 <span style="display: none;" class="an_submit_up trans" onclick="location.href='#'"><i class="fa fa-plus" aria-hidden="true"></i> 导出邮件</span></div>
	<form method="post" id="pform" action="#">
	<table class="table" cellpadding="1" cellspacing="0">
		<tr><th>ID号</th><th>E-mail</th><th>IP</th><th>优惠码 </th><th>是否使用 </th><th>时间 </th></tr>
<?php
...
?>	 
		<tr>
			...
			<td ><?php echo $row['e_ml'];?></td>
			<td bgcolor="#fafafa"><?php echo $row['e_ip'];?></td>
			<td ><?php echo $row['e_coucode'];?></td>
			<td bgcolor="#fafafa"><?php if ($row['e_flag']==1 ){echo "<font color=green>已用</font>";}else{ if($row['e_coucode']!=""){echo "<font color=red>未用</font>";}else{echo "";}}?></td>
 			<td ><?php echo $row['e_tm'];?></td>
...
<tr><td colspan="20" class="fy">
	<span class="sleft">
		...
		<input type="submit"  id="submit" value="删除选中" class="an_submit_up trans"  onclick="return chgpro('Ant_Inc.php?action=Clear&sort=dyemail','del','<?php echo $FileSelf;?>');" /></span>
